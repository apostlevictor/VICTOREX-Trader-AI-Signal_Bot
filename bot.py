...
# Deriv API functions
def deriv_api_request(method, params=None):
    """Make a request to the Deriv API"""
    try:
        url = f"https://api.deriv.com/{method}"
        if params is None:
            params = {}
...
def get_ticks_history(symbol, count=100):
    """Get tick history for a symbol"""
    deriv_symbol = DERIV_SYMBOL_MAP.get(symbol)
    if not deriv_symbol:
        return None
        
    response = deriv_api_request("ticks_history", {
...
def get_candles(symbol, granularity=60, count=100):
    """Get candle data for a symbol"""
    deriv_symbol = DERIV_SYMBOL_MAP.get(symbol)
    if not deriv_symbol:
        return None
        
    response = deriv_api_request("candles", {
        "candles": deriv_symbol,
...
def generate_signal(forex_pair, expiration_minutes):
    # Get candle data (5-minute candles, last 100 candles)
    data = get_candles(forex_pair, granularity=300, count=100)
    ...
    if user_id == ADMIN_ID and context.user_data.get('awaiting_broadcast', False):
        ...
        for uid, data in user_data.items():
            if data.get('approved', False) and uid not in banned_users:
                try:
                    await context.bob.send_message(
                        uid,
